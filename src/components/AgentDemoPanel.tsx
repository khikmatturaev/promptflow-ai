...
const DEMO_PROMPT =
    "Use PromptFlow's build_and_verify_system WebMCP tool. Design a production-ready TikTok-like platform for 10 million users with video uploads, recommendations, realtime messaging, authentication, CDN/media storage, background processing, and analytics. Infer a semantically complete component and connection plan, then let PromptFlow validate, generate, execute, break, heal, re-test, and verify the system in one call. Summarize the final architecture, real execution result, QA score, and verdict.";
...
export function AgentDemoPanel({ onClose, onNewRun }: AgentDemoPanelProps) {
    ...
    const hasOneCallRun = completedCalls.some((call) => call.toolName === "build_and_verify_system");

    const hasArchitect = hasOneCallRun || completedCalls.some((call) => call.toolName === "architect_system");
    const hasAnalyze = hasOneCallRun || completedCalls.some((call) => call.toolName === "analyze_architecture");
    const hasRecommend = hasOneCallRun || completedCalls.some((call) => call.toolName === "recommend_architecture");
    const hasStress = hasOneCallRun || completedCalls.some((call) => call.toolName === "stress_test_architecture");
    const hasTwin = hasOneCallRun || completedCalls.some((call) =>
        call.toolName === "run_digital_twin" || call.toolName === "simulate_failure" || call.toolName === "simulate_load",
    );
    const hasApply = hasOneCallRun || completedCalls.some((call) =>
        call.toolName === "apply_architecture_recommendations" || call.toolName === "apply_intelligence_recommendations",
    );
    const hasDNA = hasOneCallRun || completedCalls.some((call) => call.toolName === "get_architecture_dna");
    const hasBuild = hasOneCallRun || completedCalls.some(
        (call) => call.toolName === "assess_implementation_readiness" || call.toolName === "generate_project_blueprint",
    );
    const hasExecute = hasOneCallRun || completedCalls.some((call) => call.toolName === "run_project_execution_loop");
    const hasExplain = hasOneCallRun || completedCalls.some(
        (call) => call.toolName === "explain_architecture" || call.toolName === "generate_implementation_plan",
    );
    const hasQA = hasOneCallRun || completedCalls.some((call) => call.toolName === "run_production_qa");
    ...
    return (
        <aside className="pf-modal pf-modal-flex absolute bottom-2 right-3 z-40 flex max-h-[calc(100%-5.5rem)] w-[min(500px,calc(100%-1.5rem))] flex-col overflow-hidden rounded-lg border border-[#d9ff4f]/15 bg-[#0b0d11]/98 backdrop-blur-lg sm:right-4 sm:max-h-[min(700px,calc(100%-7rem))]">
            <div className="flex items-start justify-between gap-2 border-b border-white/10 p-2">
            ...
            <div className="overflow-y-auto p-2">
                <div className={`rounded-lg border p-2 ${toolCount > 0 ? "border-[#d9ff4f]/15 bg-[#d9ff4f]/2.5" : "border-amber-300/15 bg-amber-300/2.5"}`}>
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="rounded-full border border-white/10 bg-black/20 px-2 py-1 text-[10px] font-medium uppercase tracking-widest text-white/45">
                            {toolCount} tools · {agentOSCount}/{AGENT_OS_TOOLS.size} OS
                        </div>
                    </div>
                </div>
                ...
                <section className="mt-4">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <button
                            ...
                            className={`${copied ? 'text-green-500 cursor-not-allowed border-[#d9ff4f]/20 hover:text-[#d9ff4f]' : 'text-white/55 hover:border-[#d9ff4f]/20 hover:text-[#d9ff4f]'} cursor-pointer rounded-lg border border-white/10 bg-white/3 px-2 py-1 text-[10px]! font-medium transition`}
                        >
                            {copied ? "Copied" : "Copy prompt"}
                        </button>
                    </div>
                    <div className="mt-2 rounded-lg border border-[#d9ff4f]/10 bg-[#d9ff4f]/2.5 p-2 text-xs leading-5 text-white/65">
                        {DEMO_PROMPT}
                    </div>
                    ...
                    <button type="button" onClick={() => { const current = useCanvasStore.getState(); setFinalDemo(runFinalDemo(current.nodes, current.edges, 10_000_000)); }} disabled={useCanvasStore.getState().nodes.length === 0} className="mt-2 w-full rounded-lg cursor-pointer border border-[#d9ff4f]/20 bg-[#d9ff4f]/6 p-2 text-[10px]! font-semibold uppercase tracking-[0.14em] text-[#d9ff4f]/80 transition hover:bg-[#d9ff4f]/10 disabled:cursor-not-allowed disabled:opacity-30">Run final local demo</button>
                    {finalDemo ? (
                        <div className="mt-2 rounded-lg border border-white/8 bg-white/2 p-2">
                            <div className="flex items-center justify-between text-[8px] uppercase tracking-[0.12em]"><span className="text-white/30">Demo orchestration</span><span className={finalDemo.status === "completed" ? "text-[#d9ff4f]" : "text-red-300"}>{finalDemo.status}</span></div>
                            <div className="mt-2 grid grid-cols-2 gap-1 sm:grid-cols-4">
                                {finalDemo.stages.map((stage) => <div key={stage.id} className="rounded-md border border-white/6 bg-black/20 px-1.5 py-1 text-[8px] text-white/40"><span className="text-[#d9ff4f]/55">✓</span> {stage.title}</div>)}
                            </div>
                        </div>
                    ) : null}
                </section>
                ...
                <section className="mt-3">
                    ...
                    <div className="grid grid-cols-4 gap-1.5 sm:grid-cols-7">
                        {DEMO_STEPS.map((step, index) => (
                            <div
                                key={step.label}
                                className={`rounded-md border px-1 py-1 text-center ${stepComplete[index]
                ...
                <section className="mt-2">
                    <div className="mb-2 flex items-center justify-between gap-3">
                        ...
                        <button
                            type="button"
                            onClick={onNewRun}
                            className="rounded-lg border border-white/10 px-2 py-1 text-[10px]! cursor-pointer font-medium text-white/40 transition hover:border-white/20 hover:text-white/70"
                        >
                            New run
                        </button>
                    </div>
                    ...
                    <div className="mt-2 max-h-64 overflow-y-auto rounded-xl border border-white/8 bg-[#07080b]">
                        {toolCalls.length > 0 ? (
                            ...
                            <div className="p-3 text-center text-[10px] leading-5 text-white/25">
                ...
                <div className="mt-2 rounded-lg border border-white/8 bg-white/2 p-2 text-[10px] leading-4 text-white/30">
